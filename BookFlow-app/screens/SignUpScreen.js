...
const SignUpScreen = () => {
  ...
  const [request, response, promptAsync] = Google.useAuthRequest({
    androidClientId:
      "842894758664-481628oq82dna6jg90v2if1nn4rb411r.apps.googleusercontent.com",
    iosClientId:
      "842894758664-5lvro06b7p49mtbpgr563v9fd5fsd1eq.apps.googleusercontent.com",
    webClientId:
      "842894758664-t5fusntv19irac1qoq3dskv0ljecchgn.apps.googleusercontent.com",
    expoClientId:
      "842894758664-v1rbiib3kghprffpqr5u1kc25svb6hkf.apps.googleusercontent.com",
  });
  ...
  const send_user = async (user) => {
    try {
      const response = await fetch(`${apiUrl}/api/user/signup/googleaccount/`, {
        method: "POST",
        headers: {
    ...
    fetch(`${apiUrl}/api/token/`, {
      ...
      .then((data) => {
        ...
        if ("refresh" in data) {
          ...
          console.log(
            JSON.stringify({ password: pass, email: email, username: name })
          );
          navigation.reset({
            index: 0,
      ...
      >
        ...
        >
          <View style={[styles.containerlements]}>
            ...
            >
              <Text style={[styles.loginButton, styles.buttonText]}>
                Criar conta
              </Text>
            </TouchableOpacity>
            ...
            >
              <Text style={[styles.googleButtonbuttonText, styles.buttonText]}>
