...
public class NotificationsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(() -> {
            List<Notification> items = BookFlowDatabase.getDatabase(this).notificationDao().getAllNotifications();
            runOnUiThread(() -> {
                binding.items.setAdapter(new NotificationAdapter(items, R.layout.notification_adapter, this));
            });
            List<Notification> finalItems1 = Notification.getAllNotifications(this);
            runOnUiThread(() -> {
                binding.items.setAdapter(new NotificationAdapter(finalItems1, R.layout.notification_adapter, this));
            });
        }).start();
